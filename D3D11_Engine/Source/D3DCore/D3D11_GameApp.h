...
public:
	using FileHandler = std::function<void(const wchar_t* filePath)>;
private:
	inline static D3D11_GameApp* RunApp = nullptr;
	inline static std::unique_ptr<DefferdRenderer> MainRenderer;
public:
	/*기본 FBX 파일 핸들러*/
	static void DefaultFBXDragDropHandler(const wchar_t* filePath);
	/*기본 NodeProj 파일 핸들러*/
	static void DefaultNodeProjDragDropHandler(const wchar_t* filePath);

	/*기본으로 제공하는 핸들러로 초기화 합니다.*/
	static void SetDefaultFileHandlers();

	/*특정 확장자의 파일이 드래그 됐을때 처리할 함수를 등록합니다.*/
	static void SetFileHandlers(const wchar_t* extention, const FileHandler& EventFunc);
	static void RemoveFileHandlers(const wchar_t* extention);
	static void Present();
...
private:
	...
	std::unordered_map<std::wstring, FileHandler> fileEventHandlers;
...
private:
	struct DXGI
	{
		void Init();
		void Uninit();
		void ClearBackBuffer(const float (&color)[4]) const;
...
public:
	static DXGI& GetIDXGI() { return RunApp->IDXGI; }
};
